 ...
 * 외투기업별현황 엑셀
 */
public class PGPS0050ExcelVO implements IExcelVO {

	private static Logger logger = LoggerFactory.getLogger(PGPS0050ExcelVO.class);

    private String fileNmae = "";
    private int colIndex = 0;
    
    public PGPS0050ExcelVO() {

    }

    public PGPS0050ExcelVO(String fileName) {
        super();
    ...
    public void createExcelDocument(SXSSFWorkbook workbook, Map<String, Object> model) {
        ...
        colIndex = 0;
        // Header 생성 및 병합
        if (items[colIndex].equals("upperNm") || items[colIndex].equals("abrv")) {	// 지역 셀병합
        	sheet.addMergedRegion(new CellRangeAddress(0, 1, colIndex, colIndex));	// 헤더 병합 (시작로우, 종료로우, 시작컬럼, 종료컬럼)
        	colIndex++;
        }
        if (items[colIndex].equals("indutySeNm") || items[colIndex].equals("dtlclfcNm") || items[colIndex].equals("indutyNm")) {	// 업종 셀병합
        	sheet.addMergedRegion(new CellRangeAddress(0, 1, colIndex, colIndex));
        	colIndex++;
        }

    	sheet.addMergedRegion(new CellRangeAddress(0, 1, colIndex, colIndex));	// 고성장기준 셀병합
    	colIndex++;
        ...
        colIndex = 0;
        // Header 생성 및 병합
        if (items[colIndex].equals("upperNm") || items[colIndex].equals("abrv")) {	// 지역
        	colIndex++;
        }
        if (items[colIndex].equals("indutySeNm") || items[colIndex].equals("dtlclfcNm") || items[colIndex].equals("indutyNm")) {	// 업종
        	colIndex++;
        }
        // 고성장기준
        colIndex++;
        ...
        for (int i=0; i<subHeaders.length; i++ ) {
        	if (i % 3 == 0) sheet.addMergedRegion(new CellRangeAddress(0, 0, colIndex, colIndex+2));
        	colIndex++;
        }
