...
public class BatchJobListener implements JobListener {
	...
	public void jobToBeExecuted(JobExecutionContext jobContext) {
		...
		try {
			...
			batchResult.setUpdusr("999999999");
			batchResult.setRegister("999999999");
			...
			LOGGER.error("배치스케줄ID : {}, 배치작업ID : {}, 배치결과저장(insert) 에러 : {}"
	...
	public void jobWasExecuted(JobExecutionContext jobContext,
		...
		LOGGER.debug("job[{}] 수행시간 : {}, {}"
				, jobContext.getJobDetail().getKey().getName(), jobContext.getFireTime(), jobContext.getJobRunTime());
		...
		try {
			...
			LOGGER.error("배치결과ID : {}, 배치스케줄ID : {}, 배치작업ID : {}, 배치결과저장(update) 에러 : {}"
					, batchResult.getBatchResultId(), batchResult.getBatchSchdulId(), batchResult.getBatchOpertId(), e.getMessage());
			LOGGER.debug(e.getMessage(), e);
		}
	}
