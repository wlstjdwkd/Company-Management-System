...
public class BatchScheduler {
	...
	public void insertBatchSchdul(BatchSchdul batchSchdul) throws Exception {
		...
		if (batchSchdul.getBatchSchdulId().equals("MANUAL_ONCE")) {
			trigger = newTrigger()
				      .withIdentity(batchSchdul.getBatchSchdulId()+"."+DateUtil.getTimeNow(Calendar.getInstance().getTime()))
				      .startNow()
				      .build();
		}
		else {
		...
		if (batchSchdul.getBatchSchdulId().equals("MANUAL_ONCE")) {
			LOGGER.debug("{} 요청으로 수동실행 : {}", batchSchdul.getRegister(), batchSchdul.getBatchProgrm());
		} else {
			LOGGER.debug("{} - cronexpression : {}", batchSchdul.getBatchSchdulId(), ((CronTrigger) trigger).getCronExpression());
		}
	...
	public void init() throws Exception {
		...
		for (int i = 0; i < targetList.size(); i++) {
			...
			// 한번만 실행하는 스케줄이 과거일자이면 SKIP한다.
			if (target.getExecutCycle().equals("05")) {
				Timestamp execTm = DateUtil.getTimeStamp(target.getExecutSchdulDe()+target.getExecutSchdulHour()+target.getExecutSchdulMnt()+target.getExecutSchdulSecnd());
				if (execTm.before(now)) {
					LOGGER.debug("설정시간이 과거이므로 스케줄에서 제외");
					continue;
				}
			}
