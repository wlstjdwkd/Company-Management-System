...
var jsDeleteList = function(bbsCode){
	...
	if(selectedSeqs.length == 0){
		jsMsgBox(null, "worn", Message.template.wrongDelTarget("게시물"));
		return false;
	}

	//if(!confirm("선택한 " + selectedSeqs.length + "개의 게시물을 정말 삭제하시겠습니까?\n삭제 후 복구는 불가능 합니다.")) return false;
	jsMsgBox(null, "confirm", Message.msg.delInfo, 
		function(){
			var msg = "목록 보기 삭제";
...
var jsUpdateForm = function(pageType){
	...
	jsRequest("dataForm",_boardSvcUrl, "post");
};
...
var jsViewReq = function(page){
	//$("#dataForm input[name=q_currPage]").val(page);
	$("#df_curr_page").val(page);
	jsRequest("dataForm", _boardSvcUrl, "get");
};
...
var jsSearch = function(){
	...
	if(("" != $.trim(searchKey.val())) && ("" == $.trim(searchVal.val()))){
		jsMsgBox(null, 'info',Message.msg.searchMsg);
...
var jsView = function(bbsCd, seq, regPwd, openYn){
	...
	if((OpenWorks.isSession == false) && (regPwd != '') && (openYn == 'N')){
		$.colorbox({
			open  : true,
			title : "글 비밀번호 입력",
			href  : _boardSvcUrl + "?bbsCd=" + bbsCd + "&seq=" + seq + "&df_method_nm=boardPwdForm",
...
var jsYnAction = function(el, bbsCd, seq){
	...
	function(result){
		element.removeClass();
		if( banYn == "Y" ){
			element.addClass("icon-not-use");
			element.attr("yn", "Y");
			element.text("미사용");
		}else{
			element.addClass("icon-use");
			element.attr("yn", "N");
			element.text("사용");
		}
	}, "json");
...
var jsInsertForm = function(pageType){
	...
	if(pageType == "INSERT_ROBOT"){
		$("#df_method_nm").val("boardRtWriteForm");
	}else{
		$("#df_method_nm").val("boardWriteForm");
	}
