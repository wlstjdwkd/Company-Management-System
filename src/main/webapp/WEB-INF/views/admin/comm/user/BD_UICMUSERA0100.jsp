...
function downAttFile(fileSeq){
	...
	$().ready(function() {
		...
		$("#btn_search").click(function(){
			$("#df_curr_page").val("1");
			$("#df_method_nm").val("");
			$("#dataForm").attr('action', '/PGMS0100.do').submit();
		});
		...
		// 담당자정보 변경 적용(체크박스 전체)
		/* $("#updateProcessStep").click(function(){
			var form = document.dataForm;
			var isValidA = $('#dataForm').valid();
			
			jsMsgBox(null
				,'confirm'				
				,Message.msg.confirmChangeCharger
				,function(){
					$("#df_method_nm").val("updateProcessStep");
					$("#load_msg").text(Message.msg.ChangeCharger);
			            	...
			            	try {			            		
			            		if(response.result){
			            			if(eval(response)){
			                			...
			                			jsMsgBox(null,'info',Message.msg.failUpdate);
			                		}
			            		}else{
			            			jsMsgBox($(this),'info',Message.msg.failUpdateCharger);
			            		}
	...
	/* var fn_user_view = function(user_no) {
		...
		//$("#pMnMap.parntsMenuNo").val("48");
		//$("#pMnMap.menuNo").val("32");
