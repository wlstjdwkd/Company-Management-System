...
$(function(){
	...
	function btn_program_evaluation_view(member,PK) {
		var form = document.dataForm;
		form.df_method_nm.value = "programEvaluation";
							...
							<table cellspacing="0" border="0" summary="프로그램관리 리스트">
								...
								<tbody>
									...
									<c:forEach items="${memberList}" var="member" varStatus="status">
										<tr> 
											<td>${member.PK}</td>
											<td><a href="#" class="btn_in_gray" onclick="btn_program_evaluation_view('<c:out value="${member}"/>','<c:out value="${member.PK}"/>');"><span>${member.name}</span></a>
											</td>
											<td>${member.rank}</td>
											<td>${member.in_date}</td>
											<td></td>
										</tr>
									</c:forEach>
								</tbody>
