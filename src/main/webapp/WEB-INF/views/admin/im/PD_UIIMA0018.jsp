...
$(document).ready(function(){
	...
	$("#btn_add_bizrno").click(function(){
		...
		var appenStr = "<span id='bizBr" + index + "'><br /></span><input type='text' id='tx_bizrno_compno_first_"+ index +"' class='text tx_bizrno_compno' style='width:60px;' />"
						+ "<span id='bizFirstDash" + index + "'> - </span><input type='text' id='tx_bizrno_compno_middle_"+ index +"' class='text' style='width:40px;' />"
						+ "<span id='bizSecondDash" + index + "'> - </span><input type='text' id='tx_bizrno_compno_last_"+ index +"' class='text' style='width:80px;' />"
	...
	// 사업자등록번호 삭제
	$("#btn_delete_bizrno").click(function(){
		var index = $("input[id^='tx_bizrno_compno_first_']").length;
		if(index == 1){
			jsMsgBox(null,'info', '사업자번호는 반드시 있어야 합니다.');
			return;
		}
		$("#tx_bizrno_compno_first_" + index).remove();
		$("#tx_bizrno_compno_middle_" + index).remove();
		$("#tx_bizrno_compno_last_" + index).remove();
		$("#ad_bizrno_compno_" + index).remove();
		$("#bizFirstDash" + index).remove();
		$("#bizSecondDash" + index).remove();
		$("#bizBr" + index).remove();
		
		$("#ad_bizrno_cnt").val(index-1);
	});
	
	// 내용변경신청 수정
	$("#btn_submit").click(function(){		
		var lenCheck1 = $("#ad_entrprs_nm").val().length;
		var lenCheck2 = $("#ad_rprsntv_nm").val().length;
		var lenCheck3 = $("#ad_zip_first").val().length;
		var lenCheck4 = $("#ad_hedofc_adres").val().length;
		var lenCheck5 = $("#ad_reprsnt_tlphon_middle").val().length;
		var lenCheck6 = $("#ad_reprsnt_tlphon_last").val().length;
		
		if(lenCheck1 == 0){
			jsMsgBox(null,'info', '기업명을 입력해주세요.');
			return;
		}
		if(lenCheck2 == 0){
			jsMsgBox(null,'info', '대표자명을 입력해주세요.');
			return;
		}
		if(lenCheck3 == 0){
			jsMsgBox(null,'info', '우편번호를 입력해주세요.');
			return;
		}
		if(lenCheck4 == 0){
			jsMsgBox(null,'info', '본사주소를 입력해주세요.');
			return;
		}
		if(lenCheck5 == 0){
			jsMsgBox(null,'info', '전화번호를 입력해주세요.');
			return;
		}
		if(lenCheck6 == 0){
			jsMsgBox(null,'info', '전화번호를 입력해주세요.');
		...
		var bizLen = $("input[id^='tx_bizrno_compno_first_']").length;
		var checkBizCnt = 0;
		var checkBizBlank = false;
		var checkBizLen = false;
		for(var i = 1; i <= bizLen; i++){
			var len1 = $("#tx_bizrno_compno_first_"+i).val().length;
			var len2 = $("#tx_bizrno_compno_middle_"+i).val().length;
			var len3 = $("#tx_bizrno_compno_last_"+i).val().length;
			
			if(len1 != 0 && len2 != 0 && len3 != 0){
				checkBizCnt = checkBizCnt + 1;
			}
			...
			if(len1 != 3 || len2 != 2 || len3 != 5){
				checkBizLen = true;
			}
		}
		
		if(checkBizBlank){
			jsMsgBox(null,'info', '사업자번호에 빈칸이 있는지 확인해주세요.');
			return;
		}
		
		if(checkBizCnt < 1){
			jsMsgBox(null,'info', '사업자번호를 입력해주세요.');
			return;
		}
		
		if(checkBizLen){
			jsMsgBox(null,'info', '사업자번호의 자릿수를 확인해주세요.');
			return;
		}
		
		jsMsgBox(null,'confirm', '내용변경신청내역을 수정하시겠습니까?', 		
			function(){
				setParameter();				
				$("#df_method_nm").val("updateAK2");
				$("#dataForm").ajaxSubmit({
		            url      : "PGIM0010.do",		            
		            dataType : "text",     
		            async    : false,
		            ...
		            success  : function(response) {
		            	try {
		            		if(eval(response)){		            			
			            		jsMsgBox($(this),'info','수정이 완료되었습니다.', function(){
	...
	var initIndex = $("input[id^='tx_bizrno_compno_first_']").length;
	$("#ad_bizrno_cnt").val(initIndex);
					...
					<table class="table_form" style="margin-bottom: 20px">
						...
						<tbody>
							<tr class="table_input">
								<th class="p_l_30">기업명<a href="#none" class="entrprs_nm_dec"><img src="/images2/sub/help_btn.png" class="help_btn"></a></th>
							...
							<tr class="table_input">
								<th class="p_l_30">대표자명<a href="#none" class="btn_code rprsntv_nm_dec"><img src="<c:url value="/images2/sub/help_btn.png" />" alt="?" /></a></th>
								<td class="b_l_0"><input type="text" name="ad_rprsntv_nm" id="ad_rprsntv_nm" value="${applyEntInfo['issuBsisInfo']['RPRSNTV_NM']}" placeholder="대표자명"></td>
								<th class="p_l_30">확인서발급번호</th>
								<td class="b_l_0">${applyEntInfo['issuBsisInfo']['ISSU_NO']}</td>
							</tr>
							<tr class="table_input">
								<th class="p_l_30">본사주소<a href="#none" class="btn_code hedofc_adres_dec"><img src="<c:url value="/images2/sub/help_btn.png" />" alt="?" /></a></th>
								<td class="b_l_0" colspan="3"><input type="text" disabled name="ad_zip_first" id="ad_zip_first" class="num7" value="${applyEntInfo['issuBsisInfo']['ZIP_FIRST']}" style="width: 143px;"><a href="#none" id="btn_search_zip" class="gray_btn">우편번호 검색</a><input type="text" name="ad_hedofc_adres" id="ad_hedofc_adres" value="${applyEntInfo['issuBsisInfo']['HEDOFC_ADRES']}" style="width: 686px; margin-top: 7px;"></td>
							</tr>
							<tr>
								<th class="p_l_30">표준산업<br/>분류코드<a href="#none" class="btn_code induty_cod_dec"><img src="<c:url value="/images2/sub/help_btn.png" />" alt="?" /></a></th>
							...
							<tr class="table_input">
								...
								<td class="b_l_0">
				                    ...
				                    -<input type="text" name="ad_reprsnt_tlphon_middle" id="ad_reprsnt_tlphon_middle" class="num6" value="${applyEntInfo['issuBsisInfo']['REPRSNT_TLPHON_MIDDLE']}" style="width: 66px; margin-left: 3px; margin-right: 3px;">-<input type="text" name="ad_reprsnt_tlphon_last" id="ad_reprsnt_tlphon_last" class="num6" value="${applyEntInfo['issuBsisInfo']['REPRSNT_TLPHON_LAST']}" style="width: 66px; margin-left: 3px;">
								</td>
								...
								<td class="b_l_0">
				                    ...
				                    -<input type="text" name="ad_fxnum_middle" id="ad_fxnum_middle" class="num6" value="${applyEntInfo['issuBsisInfo']['FXNUM_MIDDLE']}" style="width: 66px; margin-left: 3px; margin-right: 3px;">-<input type="text" name="ad_fxnum_last" id="ad_fxnum_last" class="num6" value="${applyEntInfo['issuBsisInfo']['FXNUM_LAST']}" style="width: 66px; margin-left: 3px;">
								</td>
							</tr>
							<tr>
								<th class="p_l_30">사업자번호<a href="#none" class="btn_code bizrno_dec"><img src="<c:url value="/images2/sub/help_btn.png" />" alt="?" /></a><a href="#none" id="btn_add_bizrno" class="form_blBtn" style="padding:2px 8px 1px 8px; text-align:center;">사업자번호<br/>추가</a></th>
								<td class="b_l_0" id="bizrno_td" colspan="3">
									<c:forEach items="${applyEntInfo['bizrnoFormatList'] }" var="bizrnoFormat" varStatus="status">
				                		<c:if test="${not status.first }"><span id="bizBr${status.count - 1}"><br /></span></c:if>
				                			<input type="text" id="tx_bizrno_compno_first_${status.count}" class="text tx_bizrno_compno num3" value="${bizrnoFormat.first}" style="width:60px;"/><span id="bizFirstDash${status.count}"> - </span><input type="text" id="tx_bizrno_compno_middle_${status.count}" class="text num4" value="${bizrnoFormat.middle}" style="width:40px;"/><span id="bizSecondDash${status.count}"> - </span><input type="text" id="tx_bizrno_compno_last_${status.count}" class="text num5" value="${bizrnoFormat.last}" style="width:80px;"/>
				                			<input type="hidden" id="ad_bizrno_compno_${status.count}" name="ad_bizrno_compno_${status.count}" class='text' />
				                		<c:if test="${status.count eq 1}"><a href="#none" id="btn_delete_bizrno" ><span class="gray_btn">삭제</span></a></c:if>
				                	</c:forEach>
								</td>
							</tr>
							...
							<tr>
								<td class="b_l_0 t_center"><label for="type1">기업명 변경</td>
								<td class="b_l_0 t_center"><label for="type2">영위사업 양도·양수<a href="#none" class="btn_code qy_cptl_at_dec"><img src="<c:url value="/images2/sub/help_btn.png" />" alt="?" /></a></td>
								<td class="b_l_0 t_center"><label for="type3">사업자등록번호 추가</td>
							</tr>
							<tr>
								<td class="b_l_0 t_center">
									<ul class="check_sy2 check_sy2_1">
										<li>
											<input type="checkbox" id="ad_entrprs_nm_change_at" name="ad_entrprs_nm_change_at" value="Y" disabled <c:if test="${applyEntInfo['issuBsisInfo']['ENTRPRS_NM_CHANGE_AT'] == 'Y'}">checked</c:if> />
								...
								<td class="b_l_0 t_center">
									<ul class="check_sy2 check_sy2_1">
										<li>
											<input type="checkbox" id="ad_qy_cptl_at" name="ad_qy_cptl_at" value="Y" disabled <c:if test="${applyEntInfo['issuBsisInfo']['QY_CPTL_AT'] == 'Y'}">checked</c:if> />
								...
								<td class="b_l_0 t_center">
									<ul class="check_sy2 check_sy2_1">
										<li>
											<input type="checkbox" id="ad_bizrno_adit_at" name="ad_bizrno_adit_at" value="Y" disabled <c:if test="${applyEntInfo['issuBsisInfo']['BIZRNO_ADIT_AT'] == 'Y'}">checked</c:if> />
							...
							<tr class="table_input">
								...
								<td class="b_l_0">
									<div class="filebox">
										...
										--><label for="file_contract_document" class="gray_btn" style="cursor:auto;">파일선택</label>
										<input type="file" id="file_contract_document" name="fi_con_doc" class="upload-hidden upload-hidden01" disabled />
							...
							<tr class="table_input">
								...
								<td class="b_l_0">
									<div class="filebox">
										<input class="upload-name" disabled="disabled" placeholder="선택된 파일 없음" style="width: 188px;"><!-- 공백
										--><label for="file_proof_document" class="gray_btn" style="cursor:auto;" >파일선택</label>
										<input type="file" id="file_proof_document" name="fi_proof_doc" class="upload-hidden upload-hidden01" disabled />
