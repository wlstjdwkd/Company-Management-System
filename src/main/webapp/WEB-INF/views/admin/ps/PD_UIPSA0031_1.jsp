...
$(document).ready(function(){
	...
	if ( '${searchInduty}' != '') {
		...
		var induty_b2b_xport_am= document.getElementsByName("induty_b2b_xport_am");
		var induty_b2c_xport_am= document.getElementsByName("induty_b2c_xport_am");
		var induty_b2g_xport_am= document.getElementsByName("induty_b2g_xport_am");
		var induty_b2b_dome_am= document.getElementsByName("induty_b2b_dome_am");
		var induty_b2c_dome_am= document.getElementsByName("induty_b2c_dome_am");
		var induty_b2g_dome_am= document.getElementsByName("induty_b2g_dome_am");
		
		for(var i = 0; i < indutyListCnt; i++) {
			indutyTicks.push([i, indutyTick[i].value]);
			indutyD1.push([i,induty_b2b_xport_am[i].value]);
			indutyD2.push([i,induty_b2c_xport_am[i].value]);
			indutyD3.push([i,induty_b2g_xport_am[i].value]);
			indutyD4.push([i,induty_b2b_dome_am[i].value]);
			indutyD5.push([i,induty_b2c_dome_am[i].value]);
			indutyD6.push([i,induty_b2g_dome_am[i].value]);
		}
		
		if (indutyListCnt * 6 < 300 ) {
			autoscaleMargin = 0.01;
			barWidth = 0.01;
		}
		if (indutyListCnt * 6 < 100 ) {
			autoscaleMargin = 0.02;
			barWidth = 0.015;
		}		
		if (indutyListCnt * 6 < 60 ) {
			autoscaleMargin = 0.05;
			barWidth = 0.025;
		}
		if (indutyListCnt * 6 < 40 ) {
			autoscaleMargin = 0.05;
			barWidth = 0.04;			
		}
		if (indutyListCnt * 6 < 20 ) {
			autoscaleMargin = 0.1;
			barWidth = 0.09;
		}
		if (indutyListCnt * 6 < 10 ) {
			autoscaleMargin = 0.3;
			barWidth = 0.12;
		}
	...
	if( '${searchArea}' != '') {
		...
		var area_b2b_xport_am= document.getElementsByName("area_b2b_xport_am");
		var area_b2c_xport_am= document.getElementsByName("area_b2c_xport_am");
		var area_b2g_xport_am= document.getElementsByName("area_b2g_xport_am");
		var area_b2b_dome_am= document.getElementsByName("area_b2b_dome_am");
		var area_b2c_dome_am= document.getElementsByName("area_b2c_dome_am");
		var area_b2g_dome_am= document.getElementsByName("area_b2g_dome_am");

		for(var j = 0; j < areaListCnt; j++) {
			areaTicks.push([j, areaTick[j].value]);
			areaD1.push([j,area_b2b_xport_am[j].value]);
			areaD2.push([j,area_b2c_xport_am[j].value]);
			areaD3.push([j,area_b2g_xport_am[j].value]);
			areaD4.push([j,area_b2b_dome_am[j].value]);
			areaD5.push([j,area_b2c_dome_am[j].value]);
			areaD6.push([j,area_b2g_dome_am[j].value]);
		}

		if (areaListCnt * 6 < 300 ) {
			autoscaleMargin = 0.01;
			barWidth = 0.01;
		}
		if (areaListCnt * 6 < 100 ) {
			autoscaleMargin = 0.02;
			barWidth = 0.015;
		}		
		if (areaListCnt * 6 < 60 ) {
...
<form name="dataForm" id="dataForm" method="POST">
		<c:forEach items="${areaResultList}" var="areaResult" varStatus="status">
			<c:if test="${searchArea != null}">
				...
				<input type="hidden" name="area_b2b_xport_am" value="${areaResult.b2bXportAm}" />					
				<input type="hidden" name="area_b2c_xport_am" value="${areaResult.b2cXportAm}" />
				<input type="hidden" name="area_b2g_xport_am" value="${areaResult.b2gXportAm}" />
				<input type="hidden" name="area_b2b_dome_am" value="${areaResult.b2bDomeAm}" />
				<input type="hidden" name="area_b2c_dome_am" value="${areaResult.b2cDomeAm}" />
				<input type="hidden" name="area_b2g_dome_am" value="${areaResult.b2gDomeAm}" />
			</c:if>
		</c:forEach>
		...
		<c:forEach items="${indutyResultList}" var="indutyResult" varStatus="status">
			<c:if test="${searchInduty != null}">
				...
				<input type="hidden" name="induty_b2b_xport_am" value="${indutyResult.b2bXportAm}" />
				<input type="hidden" name="induty_b2c_xport_am" value="${indutyResult.b2cXportAm}" />
				<input type="hidden" name="induty_b2g_xport_am" value="${indutyResult.b2gXportAm}" />
				<input type="hidden" name="induty_b2b_dome_am" value="${indutyResult.b2bDomeAm}" />
				<input type="hidden" name="induty_b2c_dome_am" value="${indutyResult.b2cDomeAm}" />
				<input type="hidden" name="induty_b2g_dome_am" value="${indutyResult.b2gDomeAm}" />
			</c:if>
		</c:forEach>
	...
	<div class="pop_q_con">
		...
		<div class="search_top">
        	...
        	<div class="remark_20 mgt20">
	    		<ul>
	    			<li class="remark1">B2B해외</li>
	    			<li class="remark2">B2B국내</li>
	    			<li class="remark3">B2C해외</li>
	    			<li class="remark4">B2C국내</li>
	    			<li class="remark5">B2G해외</li>
	    			<li class="remark6">B2G국내</li>
	    		</ul>
	    	</div>	  
