...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#dataForm").validate({
			...
			submitHandler: function(form) {
				...
				if ($("#searchStdyySt").val() > $("#searchStdyyEd").val()) {
					jsMsgBox(null, "error", Message.msg.invalidDateCondition);
					return;
				}
				if ($("#searchStdyyEd").val() - $("#searchStdyySt").val() > 10) {
	...
	function changeTab(val) {
		...
		case 1 :
			...
			break;
		case 2 :
			$("#df_method_nm").val("selectStaticsScle");
			break;
		case 3 :
			$("#df_method_nm").val("selectStaticsHist");
			break;
		case 4 :
			$("#df_method_nm").val("selectStaticsIndx");
			break;
		default :
			$("#df_method_nm").val("");
		}
	...
	function drawChart() {
		...
		$.colorbox({
	        title : "가젤형기업분석 업력분포별추이 챠트",
	        href : "PGPS0050.do?df_method_nm=getAllChart&searchEntprsGrp="+searchEntprsGrp+"&searchStdyySt="
    		+searchStdyySt+"&ad_type="+ad_type+"&searchStdyyEd="+searchStdyyEd,
	        width : "80%",
	        height : "100%",
	        iframe : true,
	        overlayClose : false,
	...
	function printOut() {
		...
		$.colorbox({
	        title : "가젤형기업분석 업력분포별추이 출력",
	        href : "PGPS0050.do?df_method_nm=getPrintOut2&searchEntprsGrp="+searchEntprsGrp+"&searchStdyySt="
    		+searchStdyySt+"&searchInduty="+searchInduty+"&searchIndutyVal="+searchIndutyVal+"&searchArea="
    		+searchArea+"&searchAreaVal="+searchAreaVal+"&ad_type="+ad_type+"&searchStdyyEd="+searchStdyyEd,
	        width : "80%",
	        height : "100%",
	        iframe : true,
	        overlayClose : false,
...
<div id="wrap">
    <div class="wrap_inn">
        <div class="contents">
            <!--// 타이틀 영역 -->
            <h2 class="menu_title">업력별분포추이</h2>
            <!-- 타이틀 영역 //-->
			...
			<!-- 탭구분 구분 -->
			<input type="hidden" id="ad_type" name="ad_type" value="3" />
			
            <!--// tab -->
            <div class="tabwrap">
                <div class="tab">
                    <ul id="gnb">
                        <li onclick="changeTab(1);"><a href="#none">업종별분포추이</a></li><li onclick="changeTab(2);"><a href="#none">기업규모별현황</a></li><li class="on" onclick="changeTab(3);"><a href="#none">업력별분포추이</a></li><li onclick="changeTab(4);"><a href="#none">주요지표현황</a></li>
                    </ul>
                </div>
                        ...
                        <div class="search_top">
                            <table cellpadding="0" cellspacing="0" class="table_search" summary="기업군 년도 업종 지역 현황 조회 조건" >
                                ...
                                <tr>
                                    <th scope="row">기업군</th>
                                    <td>
                                    	<select id="searchEntprsGrp" name="searchEntprsGrp" style="width:100px; ">
                                    		<option value="EA5" selected>가젤형기업</option>
                                    	</select>
                                    </td>
                                    <th scope="row">년도</th>
                                    <td>
                           ...
                           <caption>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">고성장기준</th>
                                    <c:forEach var="outYr" begin="${stdyySt }" end="${stdyyEd }">
                                    <th scope="col" colspan="3">${outYr }</th>
                                    </c:forEach>
                                </tr>
                            </thead>
                            <tbody>
							<c:forEach items="${resultList }" var="resultInfo" varStatus="status">                         		
								...
								<c:if test="${not empty param.searchInduty}">
									...
									<td class='tal'>${resultInfo.resnNm }</td>
								<c:forEach var="outYr" begin="${stdyySt }" end="${stdyyEd }" varStatus="status">
									<c:set var="uname" value="y${status.count-1 }EntrHistU7" />
									<c:set var="rname" value="y${status.count-1 }EntrHistR7" />
									<c:set var="sname" value="y${status.count-1 }SumEntrHist" />
									<td class='tar'><fmt:formatNumber value="${resultInfo[uname] }"/></td>
									<td class='tar'><fmt:formatNumber value="${resultInfo[rname] }"/></td>
									<td class='tar'><fmt:formatNumber value="${resultInfo[sname] }"/></td>
								</c:forEach>
								</tr>
                           	</c:forEach>
                            </tbody>
