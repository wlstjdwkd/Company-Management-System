...
<script type="text/javascript">
	...
	function fn_egov_get_regist_view() {
		var form = document.dataForm;
		form.df_method_nm.value = "getBatchSchdulForRegist";
		form.submit();
	}
	
	function fn_egov_get_detail_view(batchOpertId) {
		var form = document.dataForm;
		form.df_method_nm.value = "getBatchSchdul";
		form.batchSchdulId.value = batchOpertId;
		form.submit();
	}
...
<body>
	<div id="wrap">
		<div class="wrap_inn">
			<div class="contents">
				<!--// 타이틀 영역 -->
				<h2 class="menu_title">배치스케줄관리</h2>
				<!-- 타이틀 영역 //-->
                ...
                <div class="block">
                    ...
                    <!--// 리스트 -->
                    <div class="list_zone">
                        <table cellspacing="0" border="0" summary="배치스케줄ID 배치작업명 배치프로그램 실행주기 실행스케줄 리스트">
                            <caption>
                            배치작업리스트
                            </caption>
                            <colgroup>
                        ...
                        <col width="13%" />
							...
							<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
								<tr>
									<td><a href="#none" onclick="fn_egov_get_detail_view('<c:out value='${resultInfo.batchSchdulId}'/>')">
												<c:out value='${resultInfo.batchSchdulId}' /></a>
									</td>
									<td class="tal">${resultInfo.batchOpertNm}</td>
									<td class="tal">${resultInfo.batchProgrm}</td>
									<td>${resultInfo.executCycleNm}</td>
									<td class="tal">${resultInfo.executSchdul}</td>
								</tr>
							</c:forEach>
							</tbody>
						</table>
                    ...
                    <div class="btn_page_last">
                    	<a class="btn_page_admin" href="#none" onclick="fn_egov_get_regist_view()"><span>등록</span></a>
                    </div>
