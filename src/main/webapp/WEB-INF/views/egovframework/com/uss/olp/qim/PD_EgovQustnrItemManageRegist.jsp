	...
	$("#qustnrItemManageVO").validate({
		...
		submitHandler: function(form) {
			...
			jsMsgBox(null, 'confirm','<spring:message code="confirm.common.save" />', function(){
				$("#cmd").val("save");
				$("#df_method_nm").val("insertEgovQustnrItemManage");
				form.submit();
			});
		}
	});	
...
function checkSn() {
	$.ajax({
		...
		type     : "POST",
		dataType : "json",
		data     : { df_method_nm: "EgovQustnrItemCheckSn", qestnrId: $("#qestnrId").val(), qestnrTmplatId: $("#qestnrTmplatId").val(), qestnrQesitmId: $("#qestnrQesitmId").val(), iemSn: $("#iemSn").val() },
...
<div id="self_dgs">
	<div class="pop_q_con">	
		<div>
			...
			<input type="hidden" name="qestnrTmplatId" id="qestnrTmplatId" value="${listQustnrTmplat[0].qestnrTmplatId}" />
			<input type="hidden" name="qestnrId" id="qestnrId" value="${listQustnrTmplat[0].qestnrId}" />
			
			<input type="hidden" name="cmd" id="cmd" value="">
			
	         <table cellpadding="0" cellspacing="0" class="table_basic" summary="설문항목 등록">
	             <caption>
	                 설문항목 등록
	             </caption>
	             <colgroup>
	             ...
	             <tbody>
	                 <tr>
	                     ...
	                     <td><c:out value="${fn:replace(listQustnrTmplat[0].qustnrSj , crlf , '<br/>')}" escapeXml="false" /></td>
	                 </tr>
	                 <tr>
	                     <th scope="row">설문문항</th>
	                     <td><c:out value="${fn:replace(listQustnrTmplat[0].qestnCn , crlf , '<br/>')}" escapeXml="false" /></td>
	                 </tr>
	                 <tr>
	                     <th scope="row" class="point">항목순번</th>
	                     <td>
	                           	<input type="text" name="iemSn" id="iemSn" style="width:110px;" title="항목순서입력" onfocusout="checkSn();" />
	                 ...
	                 <tr>
	                     ...
	                     <td>
	                          	<textarea name="iemCn" id="iemCn" rows="8" title="내용입력란"  style="width:90%;" placeholder="항목내용을 입력해 주세요."></textarea>
