...
<head>
	...
	$().ready(function(){
	    ...
	    $('#q_startDt').datepicker({
            showOn : 'button',
            defaultDate : null,
            buttonImage : '<c:url value="/images2/sub/icon_cal.gif" />',
				...
				<div class="r_cont">
					...
					<form name="dataForm" id="dataForm" method="post" >
						...
						<div class="sear_sec st_box01">
							...
							<select class="select_box" name="q_searchKeyType" id="q_searchKeyType" style="width:180px;vertical-align:top;" onchange="if($(this).val() != ''){$(this).next().focus().select();}">
							...
							<input type="hidden" name="q_searchKey" value="${param.q_searchKey}" />
							<input type="text" name="q_searchVal" id="q_searchVal" value="${param.q_searchVal}" title="검색어를 입력하세요." placeholder="    검색어를 입력하세요." onfocus="this.placeholder=''; return true" maxlength="50" style=" width:335px;<c:if test="${param.df_menu_no eq 12}">width:535px;</c:if>  height: 35px; border: 1px solid #dadada; vertical-align: middle; " onkeyup="if(event.keyCode==13)jsSearch();">
						...
						<div class="table_list">
							...
							<table class="st_bl01">
									...
									<tr>								
										...
										<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="status">
											<c:choose>
												...
												<c:when test="${arrange.columnId eq 'REG_NM'}">
												</c:when>
												<c:otherwise>
											...
											<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="arrStatus">
												<c:choose>
													<c:when test="${arrange.columnId eq 'TITLE'}">
														...
														<c:otherwise>
														<td class="tw txt_l">
															<a href="#none" onclick="jsView('${noticeVo.bbsCd}', '${noticeVo.seq}', '${noticeVo.regPwd}', '${noticeVo.openYn}'); return false;">
																<b>${noticeVo.title}</b>
								...
								<c:forEach items="${boardList}" var="baseVo" varStatus="status">
									<tr>
										<!-- 번호(필수) -->
										<td class="txt_l" style="text-align:center;padding-left:0;">
											<c:choose>
