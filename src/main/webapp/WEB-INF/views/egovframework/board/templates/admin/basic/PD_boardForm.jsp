	...
	$(document).ready(function(){
		...
		SharedAhpek.jsSetFormValidate = function(){
			...
			var submitFunction = function(form){
			  	...
			  	//파일수 초과 체크
                if($("#multiFiles_wrap_list .MultiFile-remove").length + $("#fileTd #fileImg").length > Ahpek.maxFileCnt){
				    alert("최대 업로드파일수를 초과하였습니다.");
				    return;
				}
		...
		<div class="pop_q_con">
			...
			<table class="table_basic" cellspacing="0" border="0" summary="게시판 내용 작성페이지입니다.">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="text" name="title" id="title" style="width:99%" title="제목을 입력해주세요" value="${dataVo.title}" />
					...
					<c:if test="${boardConfVo.ctgYn eq 'Y' or boardConfVo.noticeYn eq 'Y' or boardConfVo.openYn eq 'Y'}">
						<tr>
							<th scope="row" class="point"><label for="basicConf">기본 설정</label></th>
							<td>
								<c:if test="${boardConfVo.ctgYn eq 'Y'}">
					...
					<c:if test="${boardConfVo.fileYn eq 'Y'}" >
						<tr>
							<th rowspan="2" scope="row"><label for="file">파일첨부</label></th>
							<td class="rows" id="fileTd">
								<c:if test="${pageType eq 'UPDATE'}">
									<c:if test="${fn:length(dataVo.fileList) > 0}">
										...
										<c:forEach items="${dataVo.fileList}" var="fileVo">
											<div>													
												<a href="#" onclick="jsFileDelete(this, ${fileVo.fileSeq}, '${fileVo.fileId}');"><img src="<c:url value="/images/acm/icon_del.png" />" id="fileImg" /></a>
			...
			<%-- <div class="btn_r">
				<ul>
					...
					<li><input type="button" value="목록" style="cursor:pointer;" class="blue" onclick="jsList('${param.df_curr_page}');" /></li>
				</ul>
			</div> --%>
